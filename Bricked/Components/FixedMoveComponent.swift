...
var moveSound = SKAction.playSoundFileNamed(GameConstants.MoveSoundFileName, waitForCompletion: false)
var adjustSound = SKAction.playSoundFileNamed(GameConstants.AdjustSoundFileName, waitForCompletion: false)
...
class FixedMoveComponent: GKComponent {
  ...
  func drop() {
    ...
    if collider.canDrop {
      ...
      polyominoComponent.spriteComponents.first!.sprite.run(moveSound)
    }
    else {
